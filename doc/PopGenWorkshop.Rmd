...
each section below using the `mkdir` command. Finally the `results` subdirectory
...
```bash
plink --bfile ../data/H938_Euro --hardy --chr 2 --out H938_Euro --noweb
```
...
```bash
grep ALL H938_Euro.hwe > H938_Euro.hwe_reduced
```
...
3. Now, go through the same steps for the full H938 set of plink files (since the files were too big upload to github, please look at `results_answerkey/HWE/H938.hwe_reduced` in order to make your plot). Compare the deficiency in heterozygotes between the world-wide data and the European only data. In which is the deficiency smaller? Why might that be the case?
